$(function () {
	...
	// console.log("elemnt width start" + element.width());
	...
	if (!on_mobile) {
		...
		$("#barba-wrapper").on("mousewheel", ".project", function (e, delta) {
			if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
				//if you're scrolling more vertically than horizontally
	...
	scroll_element.scroll(function () {
		//gotta get this to work when you switch to a new one

		if (this.scrollLeft + window.innerWidth * 2 > scroll_width) {
			//if you're close to the end of the scroll
			// console.log("reach call point!");
			if (callable) {
				//if call_images hasn't been called for this scroll event yet
				call_images(url_json);
			}
		}
	});

	$("#name_mobile").click(function () {
		//when u click "lucas vasilko" on mobile
		if (small_screen) {
			//if small screen
	...
	$("li a").click(function (event) {
		//makes menu disappear on mobile when you click a project
		if (small_screen) {
			//if small screen
			$(".header").toggleClass("header_open");
	...
	$("#name").mouseenter(function () {
		//makes menu appear when you hover over name
	...
	$(".header").mouseleave(function () {
		//makes menu disappear when you leave the area
		if (about_visible && !small_screen) {
			$(".header").toggleClass("header_open");
			about_visible = false;
		}
	...
	load_more.click(function () {
		//when u click the load more button
		if (more_images) {
			call_images(url_json);
		}
	});
	...
	$("img").hover(
		function (event) {
			if (!on_mobile) {
				//if not on mobile
				if ($(this).parent().has("figcaption").length) {
	...
	function call_images(jsonurl, delay = 500) {
		...
		if (more_images) {
			...
			$.get(jsonurl + "/offset:" + offset, function (data) {
				if (data.more === false) {
					//if there aren't any more images to load, delete button
					// $('#load_more').hide();
					// console.log("no more!");
					more_images = false;
	...
	function update_scroll_width(delay) {
		setTimeout(function () {
			//wait for a sec for the images to settle
	...
	function reload_functions() {
		...
		scroll_element.scroll(function () {
			//this is what makes it work when you switch barbas
			if (this.scrollLeft + window.innerWidth * 2 > scroll_width) {
				// console.log("reach call point!");
				if (callable) {
					//if it hasn't been called for this scroll event yet
					call_images(url_json);
				}
			}
		});
